...
const ParticleMap = () => {
  ...
  useEffect(() => {
    ...
    class Particle {
      constructor(x, y) {
        ...
        this.velX = Math.random() * (velocity * 2) - velocity;
        this.velY = Math.random() * (velocity * 2) - velocity;
        this.nbrOfLinks = 0;
      }
      ...
      update() {
        const posRelativeToCenter = {
          x: this.x - this.centerX,
          y: this.y - this.centerY
        };

        const distance = Math.sqrt(Math.pow(posRelativeToCenter.x, 2) + Math.pow(posRelativeToCenter.y, 2));

        if (distance > particleZone) {
          const force = (particleZone - distance) / particleZone;
      ...
      renderLinks(context, particles) {
        particles.forEach(p => {
          if (this.nbrOfLinks > linkMax) return;

          const distance = Math.sqrt(Math.pow(this.x - p.x, 2) + Math.pow(this.y - p.y, 2));

          if (distance < distanceLinkMax && this.nbrOfLinks < linkMax && p.nbrOfLinks < linkMax) {
            context.beginPath();
    ...
    for (let i = 0; i < numberParticlesStart; i++) {
      const x = jsonMap[i].cx;
      const y = jsonMap[i].cy;
    ...
    const loop = () => {
      ...
      particles.forEach(p => p.renderLinks(context, particles.filter(particle => particle !== p)));
      particles.forEach(p => {
        p.render(context);
        if (showZone) p.renderZone(context);
      });
  ...
  return (
    <div id="js-map-canvas" className="map-canvas">
      <canvas
        ref={canvasRef}
        id="canvas-map"
        width={1440}
      ...
      />
      <img 
        src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/16721/map-canvas-fallback.png" 
        alt="Map Fallback" 
      />
    </div>
  );
